			...
			<table>
				<tr>
					<td><input type="text" class="form-control time-select"
						id="time1" value="10:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time2" value="11:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time3" value="12:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time4" value="13:00 예약가능" readonly /></td>
				</tr>
				<tr>
					<td><input type="text" class="form-control time-select"
						id="time5" value="14:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time6" value="15:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time7" value="16:00 예약가능" readonly /></td>
					<td><input type="text" class="form-control time-select"
						id="time8" value="17:00 예약가능" readonly /></td>
				</tr>
			</table>
	        ...
	        $.ajax({
	            ...
	            success: function(data) {  // 서버 응답이 성공적일 경우 실행
	                ...
	                timeInputs.forEach(function(timeInput) {
	                    ...
	                    $(timeInput).css('color', '');  // 모든 요소의 색상 초기화
               			timeInput.style.textDecoration = '';  // 줄 그음 제거
	                    timeInput.value = timeInput.value.replace(' 예약마감', ' 예약가능');  // 예약마감 텍스트를 예약가능으로 복구
	                ...
	                data.forEach(function(reservedTime) {
	                    timeInputs.forEach(function(timeInput) {
	                        ...
	                        if (reservedTime === timeInput.value.split(' ')[0]) {  // 예약된 시간과 일치하는 경우
	                       		$(timeInput).css('color', 'crimson');  // 예약마감 텍스트를 붉은색으로
	                            timeInput.style.textDecoration = 'line-through'; // 시간에 줄 그음 추가
	                            timeInput.classList.add('red-text');  // 예약마감 텍스트를 붉은색으로
	                            timeInput.value = timeInput.value.replace(' 예약가능', ' 예약마감');
	                            timeInput.disabled = true;  // 실제로 입력 필드를 비활성화
	                        }
	                    });
	                });
