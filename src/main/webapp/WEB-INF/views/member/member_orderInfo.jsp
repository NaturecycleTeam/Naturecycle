				...
				<tbody>
					...
					<c:if test="${list.size() != 0}">
						<c:forEach var="dto" items="${list}">
							<c:if test="${dto.order_count != 0}">
								<tr>
									<td>${dto.order_num}</td>
									<td><fmt:formatDate value="${dto.order_date}" pattern="yyyy-MM-dd HH:MM"/></td>
									<td><a href="myOrderDetail.do?order_num=${dto.order_num}">${dto.pname} 외 ${dto.order_count}건</a></td> 
									<td><fmt:formatNumber value="${dto.TOT_PURCHASE_AMOUNT}" type="currency"/> </td>								
								</tr>
							</c:if>
							<c:if test="${dto.order_count == 0}">
								<tr>
									<td>${dto.order_num}</td>
									<td><fmt:formatDate value="${dto.order_date}" pattern="yyyy-MM-dd HH:MM"/></td>
									<td><a href="myOrderDetail.do?order_num=${dto.order_num}">${dto.pname}</a></td> 
									<td><fmt:formatNumber value="${dto.TOT_PURCHASE_AMOUNT}" type="currency"/> </td>								
								</tr>
							</c:if>
						</c:forEach>
